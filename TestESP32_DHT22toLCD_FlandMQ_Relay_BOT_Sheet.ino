...
//28-11-68 มีการเปลี่ยนการทำงานของตัววัดอุณหภูมิ
...
#define GSHEET_URL "https://script.google.com/macros/s/AKfycbySTvVzsh3qmP9L-zbxJu7O-vsuEkHkVLZbS9z8_3wYhjohOWPkOzKr60a-hMSTiYwV/exec"
...
void SendTelegramTask() {
  ...
  if (!isnan(ac1Power) && ac1Power > 300) {
    ...
    if (!tgAC1OffSent) {
      bot.sendMessage(
        CHAT_ID,
        "⚠️ เครื่องปรับอากาศ A1 หยุดทำงาน หรือไม่มีการใช้พลังงาน",
        "");
      tgAC1OffSent = true;
      tgAC1OnSent = false;
    }
  ...
  if (!isnan(ac2Power) && ac2Power > 300) {
    ...
    if (!tgAC2OffSent) {
      bot.sendMessage(
        CHAT_ID,
        "⚠️ เครื่องปรับอากาศ A2 หยุดทำงาน หรือไม่มีการใช้พลังงาน",
        "");
      tgAC2OffSent = true;
      tgAC2OnSent = false;
    }
...
void SendDataToSheet() {
  ...
  String payloadTH =
    "{\"type\":\"temp_humi\","
    "\"temp\":" + String((int)lastTemp) + ","
    "\"humi\":" + String((int)lastHumi) + "}";
...
void setup() {
  ...
  //เวลาที่AIคำนวณแล้วว่าชนกันน้อยที่สุด
  ...
  TimerTempHumiSheet.setInterval(60000L, SendDataToSheet);
}
